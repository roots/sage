 ...
 * @todo https://github.com/retlehs/shoestrap/issues/461
 *
 * Rewrite:
 ...
 *
 * @link https://github.com/retlehs/shoestrap/wiki/Nginx
 * @link https://github.com/retlehs/shoestrap/wiki/Lighttpd
 */
...
if (stristr($_SERVER['SERVER_SOFTWARE'], 'apache') || stristr($_SERVER['SERVER_SOFTWARE'], 'litespeed') !== false) {
  ...
  function shoestrap_htaccess_writable() {
    if (!is_writable(get_home_path() . '.htaccess')) {
      if (current_user_can('administrator')) {
        add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(__('Please make sure your <a href="%s">.htaccess</a> file is writable ', 'shoestrap'), admin_url('options-permalink.php')) . "</p></div>';"));
      }
    }
  }

  add_action('admin_init', 'shoestrap_htaccess_writable');
  ...
  function shoestrap_clean_urls($content) {
