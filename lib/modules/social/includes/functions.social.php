	...
	foreach ( $networks as $network ) {
		if ( $network['on'] == 1 ) {
			...
			elseif ( $network['icon'] == 'twitter' )
				$url .= ( $twittername != "" ) ? ' via @' . $twittername : $url    = 'http://twitter.com/home/?status=' . get_the_title() . ' - ' . get_permalink();
...
function shoestrap_add_social_actions() {
	...
	if ( $social_sharing_single_post == 1 ) {
		if ( $social_sharing_location == 'top' ) {
	...
	if ( $social_sharing_single_page == 1 ) {
		if ( $social_sharing_location == 'top' ) {
