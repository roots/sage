...
if (stristr($_SERVER['SERVER_SOFTWARE'], 'apache') !== false) {
  ...
  function roots_htaccess_writable() {
	...
	function roots_add_rewrites($wp_rewrite) {
	...
	function roots_add_h5bp_htaccess($wp_rewrite) {
		if ( is_multisite() )
			return;
		...
		if ((!file_exists($htaccess_file) && is_writable($home_path) && $wp_rewrite->using_mod_rewrite_permalinks()) || is_writable($htaccess_file)) {
			if ( got_mod_rewrite() ) {
				$h5bp_rules = extract_from_markers( $htaccess_file, 'HTML5Boilerplate' );						
			    if( $h5bp_rules === array() ) {
				    $filename = __DIR__ . '/h5bp-htaccess';    
					return insert_with_markers( $htaccess_file, 'HTML5Boilerplate', extract_from_markers( $filename, 'HTML 5 Boilerplate' ) );
			    }
			}
		}
		return false;
