...
if (is_admin() && $pagenow  === 'themes.php' && isset( $_GET['activated'])) {
        wp_redirect('/wp-admin/themes.php?page=theme_activation_options');
        exit();
}

function roots_theme_activation_options_init() {
  if (false === roots_get_theme_activation_options())
...
function roots_theme_activation_options_add_page() {
  ...
  if(!$roots_activation_options['first_install_done']){   
    ...
    if (is_admin() && $_GET['page'] == 'theme_activation_options'){          
...
function roots_theme_activation_options_render_page() { ?>
	...
	<div class="wrap">
		...
		<form method="post" action="options.php">
			...
			?>
                        <input type="hidden" value="1" name="roots_theme_activation_options[first_install_done]" />
...
function roots_theme_activation_action(){
    ...
    //set every options to false in case this function is run again (shouldn't be but who knows...)
