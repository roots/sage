...
if ( ! class_exists( 'Shoestrap_Options' ) ) {
	...
	class Shoestrap_Options {
		...
		public function setSections() {
			...
			$customize_title = sprintf( __( 'Customize &#8220;%s&#8221;', 'shoestrap'), $this->theme->display('Name'));
			...
			<div id="current-theme" class="<?php echo esc_attr($class); ?>">
				...
				<div>
					<ul class="theme-info">
						<li><?php printf( __( 'By %s', 'shoestrap' ), $this->theme->display( 'Author' ) ); ?></li>
						<li><?php printf( __( 'Version %s', 'shoestrap' ), $this->theme->display( 'Version' ) ); ?></li>
						<li><?php echo '<strong>' . __( 'Tags', 'shoestrap' ) . ':</strong> '; ?><?php printf( $this->theme->display( 'Tags' ) ); ?></li>
					</ul>
					<p class="theme-description"><?php echo $this->theme->display( 'Description' ); ?></p>

					<?php
					if ( $this->theme->parent() ) {
						printf('<p class="howto">' . __( 'This <a href="%1$s">child theme</a> requires its parent theme, %2$s.' ) . '</p>', __( 'http://codex.wordpress.org/Child_Themes', 'shoestrap' ), $this->theme->parent()->display( 'Name' ) );
					}
					?>
				</div>
			</div>
